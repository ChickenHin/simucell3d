#!/bin/bash
################################################################################
# 24-Hour Performance Comparison: v1.0 vs Current (Adaptive)
#
# This script launches a 24-hour performance comparison between v1.0 and
...
echo -e "${BLUE}════════════════════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  24-Hour Performance Comparison: v1.0 vs Current (Adaptive)${NC}"
echo -e "${BLUE}════════════════════════════════════════════════════════════════════════════${NC}"
echo ""
...
OUTPUT_DIR="$PROJECT_ROOT/doc/working/24hour_comparison_$(date +%Y%m%d_%H%M%S)"
...
# Check disk space (need at least 15GB for 24-hour run)
...
if [ "$FREE_SPACE_GB" -lt 15 ]; then
    ...
    echo "  Recommendation: At least 15GB free for 24-hour run"
...
cp "$PROJECT_ROOT/$PARAM_FILE" /tmp/params_v1_24hour.xml
cp "$PROJECT_ROOT/$PARAM_FILE" /tmp/params_current_24hour.xml

# Update paths for v1.0
sed -i "s|<input_mesh_file_path>.*</input_mesh_file_path>|<input_mesh_file_path>$MESH_ABS_PATH</input_mesh_file_path>|g" /tmp/params_v1_24hour.xml
sed -i "s|<output_mesh_folder_path>.*</output_mesh_folder_path>|<output_mesh_folder_path>$OUTPUT_DIR/sim_v1</output_mesh_folder_path>|g" /tmp/params_v1_24hour.xml

# Update paths for current
sed -i "s|<input_mesh_file_path>.*</input_mesh_file_path>|<input_mesh_file_path>$MESH_ABS_PATH</input_mesh_file_path>|g" /tmp/params_current_24hour.xml
sed -i "s|<output_mesh_folder_path>.*</output_mesh_folder_path>|<output_mesh_folder_path>$OUTPUT_DIR/sim_current</output_mesh_folder_path>|g" /tmp/params_current_24hour.xml
...
nohup timeout $DURATION_SECONDS taskset -c $V1_CORES \
  /usr/bin/time -v ./simucell3d /tmp/params_v1_24hour.xml \
...
if [ -f "$SCRIPT_DIR/monitor_24hour.sh" ]; then
    nohup "$SCRIPT_DIR/monitor_24hour.sh" "$OUTPUT_DIR" > "$OUTPUT_DIR/monitor_output.log" 2>&1 &
    MONITOR_PID=$!
...
# 24-Hour Performance Comparison - IN PROGRESS
...
The simulations will automatically terminate after 24 hours or when they reach 128,000 iterations (whichever comes first).

Based on previous benchmarks (~1.54x speedup), we expect:
- **v1.0**: Will complete all 128,000 iterations (estimated ~18-20 hours)
- **Current**: Will complete all 128,000 iterations much faster (estimated ~12-14 hours)

Both should reach the 128k iteration limit before the 24-hour timeout, providing a complete benchmark comparison.

---
Generated by launch_24hour_comparison.sh
EOF
...
echo ""
echo -e "${GREEN}════════════════════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}  ✓ 24-Hour Comparison Successfully Launched!${NC}"
echo -e "${GREEN}════════════════════════════════════════════════════════════════════════════${NC}"
echo ""
...
echo ""
echo -e "${YELLOW}Expected completion: $(date -d '+24 hours')${NC}"
echo ""
echo -e "${BLUE}Note:${NC} Both simulations may complete before 24 hours if they reach 128k iterations."
echo "      Check logs periodically to see when they finish."
echo ""
