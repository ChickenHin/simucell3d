#!/bin/bash
################################################################################
# Pre-Flight Check for 12-Hour Performance Comparison
#
...
echo -e "${BLUE}════════════════════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Pre-Flight Check: 12-Hour Performance Comparison${NC}"
echo -e "${BLUE}════════════════════════════════════════════════════════════════════════════${NC}"
echo ""
...
if [ "$FREE_SPACE_GB" -ge 10 ]; then
    echo -e "${GREEN}PASS${NC} (10GB+ recommended)"
    PASS=$((PASS+1))
elif [ "$FREE_SPACE_GB" -ge 5 ]; then
    echo -e "${YELLOW}WARN${NC} (10GB+ recommended, found ${FREE_SPACE_GB}GB)"
    WARN=$((WARN+1))
else
    echo -e "${RED}FAIL${NC} (10GB+ recommended, found ${FREE_SPACE_GB}GB)"
    FAIL=$((FAIL+1))
fi
...
if [ -f "$SCRIPT_DIR/launch_12hour_comparison.sh" ]; then
    echo -e "${GREEN}PASS${NC}"
    PASS=$((PASS+1))
else
    echo -e "${RED}FAIL${NC}"
...
if [ -f "$SCRIPT_DIR/launch_12hour_comparison.sh" ]; then
    ...
    if [ -x "$SCRIPT_DIR/launch_12hour_comparison.sh" ]; then
        echo -e "${GREEN}PASS${NC}"
        PASS=$((PASS+1))
    else
        echo -e "${YELLOW}WARN${NC}"
...
if [ "$FAIL" -eq 0 ]; then
    ...
    echo "  cd $SCRIPT_DIR"
    echo "  ./launch_12hour_comparison.sh"
    echo ""
    exit 0
